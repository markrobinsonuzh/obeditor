...
const PREVIEW_PY = [
  ...
  "    all_nodes = []",
  "    for _target in _g.nodes:",
  "        if _target in _initial_set:",
  "            _paths = [[_target]]",
  "        else:",
  "            _paths = []",
  "            for _s in _initial:",
  "                _paths.extend(all_simple_paths(_g, _s, _target))",
  "        for _path in _paths:",
  "            _prev = None",
  "            for _cn in _path:",
  "                _prev = _ob_node(_prev, _cn)",
  "            all_nodes.append(_prev)",
  "    # rule all",
  "    out.write('rule all:\\n    input:\\n')",
  ...
  "        return {'ok': True, 'snakefile': snakefile, 'runner': _generate_runner(bench, yaml_content, canonical_url),",
  "                'stats': json.dumps(stats), 'mermaid': mermaid, 'mermaid_links': mermaid_links, 'io_summary': io_summary,",
...
async function convert(yaml) {
  ...
  try {
    ...
    result.destroy();
    self.postMessage({ type: "result", ok: obj.ok, snakefile: obj.snakefile, runner: obj.runner, stats: obj.stats, mermaid: obj.mermaid, mermaid_links: obj.mermaid_links, io_summary: obj.io_summary, error: obj.error, line: obj.line, wizard: obj.wizard, parent_hash: obj.parent_hash, parent_url: obj.parent_url });
  } catch (e) {
